...
%                   as atomdata(:,:,t)
% .data         --> struct with vectors of each atomdata with variable by their name
% 
% Example
...
while feof(dump) == 0
    id = fgetl(dump);
     ...
     if (strcmpi(id,'ITEM: NUMBER OF ATOMS')|strcmpi(id,'ITEM: NUMBER OF ENTRIES'))
      ...
      if (strcmpi(id(1:16),'ITEM: BOX BOUNDS'))
       ...
       if (strcmpi(id(1:11),'ITEM: ATOMS')|strcmpi(id(1:13),'ITEM: ENTRIES'))
            names=readwords(id);
            items=length(names)-3;
            for j = 1 : 1: Natoms
                atom_data(j,:,i) = str2num(fgetl(dump));
            end
            data=struct;
            for k=1:1:items
                tempname=['data.',names{k+2,1}];
                sign=find(tempname=='[');tempname(sign)=[];
                sign=find(tempname==']');tempname(sign)=[];
                sign=find(tempname=='+');tempname(sign)=[];
                sign=find(tempname=='-');tempname(sign)=[];
                sign=find(tempname=='*');tempname(sign)=[];
                sign=find(tempname=='/');tempname(sign)=[];
                eval([tempname '=atom_data(:,k)']);
            end
            i=i+1;
       end
      end 
